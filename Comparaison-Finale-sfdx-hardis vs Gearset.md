...
## 10C. sfdx-hardis Org Monitoring

### Overview

sfdx-hardis provides an **Org Monitoring feature** that creates a dedicated **Git repository** for daily metadata backups. This is a **separate Git repository** (not a separate Salesforce org) that backs up your **existing orgs** (Production, UAT, INT).

```mermaid
flowchart TB
    subgraph "Same Salesforce Orgs"
        PROD[(Production)]
        UAT[(UAT)]
        INT[(Integration)]
    end
    
    subgraph "Two Separate Git Repositories"
        subgraph "CI/CD Repository"
            CICD_MAIN[main branch]
            CICD_INT[integration branch]
            CICD_UAT[uat branch]
            CICD_DEV[feature branches]
        end
        
        subgraph "Monitoring Repository"
            MON_PROD[monitoring_production branch]
            MON_UAT[monitoring_uat branch]
            MON_INT[monitoring_integration branch]
        end
    end
    
    PROD -->|Nightly Backup| MON_PROD
    UAT -->|Nightly Backup| MON_UAT
    INT -->|Nightly Backup| MON_INT
    
    CICD_MAIN -->|Deploy| PROD
    CICD_UAT -->|Deploy| UAT
    CICD_INT -->|Deploy| INT
```

> **Key Point**: No additional Salesforce orgs are needed. The monitoring repository simply provides a **second Git repository** that stores nightly snapshots of your existing orgs' metadata.

### CI/CD vs Monitoring Repository

| Aspect | CI/CD Repository | Monitoring Repository |
|--------|------------------|----------------------|
| **Purpose** | Source of truth for deployments | Backup & audit trail |
| **Content** | Curated metadata for deployment | Full org metadata snapshot |
| **Updates** | Developer commits via PR | Automated nightly commits |
| **Branch Strategy** | integration → uat → preprod → main | One branch per monitored org |
| **Triggers** | PRs, merges, manual | Scheduled (cron) |
| **Notifications** | PR comments, deployment status | Slack/Teams change alerts |
...
sequenceDiagram
    ...
    participant GIT as Monitoring Repo
    participant SLACK as Slack/Teams
    
    CRON->>CI: Trigger nightly job
    CI->>SF: sf hardis:auth:login
    CI->>SF: sf hardis:org:monitor:backup
    SF-->>CI: All metadata retrieved
    CI->>GIT: git add --all
    CI->>GIT: git commit (if changes)
    CI->>GIT: git push
    
    alt Changes detected
        CI->>SLACK: Send notification with changed metadata list
    end
    
    Note over CI: Additional checks run after backup
    CI->>SF: Run Apex tests
    CI->>CI: Run MegaLinter (code quality)
    CI->>SF: Check deprecated API calls
    CI->>SF: Check org limits
```

### Monitoring Repository Structure

```
monitoring-repo/
├── monitoring_production/          # Branch for PROD org
│   ├── force-app/
│   │   └── main/
│   │       └── default/
│   │           ├── classes/
...
│   └── .sfdx-hardis.yml
│
├── monitoring_uat/                 # Branch for UAT org
│   └── ...
│
└── monitoring_integration/         # Branch for INT org
    └── ...
```

### Monitoring Checks (Out of the Box)

| Check | Frequency | Description |
|-------|-----------|-------------|
| **Metadata Backup** | Daily | Full metadata snapshot to git commit |
| **Suspect Setup Actions** | Daily | Detect suspicious audit trail entries |
| **Apex Tests** | Daily | Run all Apex tests |
| **MegaLinter Quality** | Daily | Static analysis of metadata |
| **Org Limits** | Daily | Check API limits, storage, etc. |
| **Deprecated API Calls** | Daily | Detect calls to deprecated API versions |
| **Release Updates** | Weekly | Check Salesforce release updates |
| **Inactive Users** | Weekly | Detect users with no recent login |
| **Unused Licenses** | Weekly | Detect unused license assignments |
| **Unused Apex Classes** | Weekly | Detect unused Batch/Schedulable/Queueable |
| **Unused Connected Apps** | Weekly | Detect unused OAuth apps |
| **Missing Access** | Weekly | Elements with no permission set access |
| **Inactive Metadata** | Weekly | Flows, validation rules, etc. marked inactive |

### Monitoring vs Retrofit: Key Distinction

**Important**: The monitoring repository is **NOT** the source for retrofit. They serve different purposes:

```mermaid
flowchart TB
    subgraph "RETROFIT Process (CI/CD Repo)"
        PROD_ORG[(Production Org)]
        RETROFIT_JOB[Retrofit GitHub Action]
        RETROFIT_PR[PR to retrofitBranch]
        CICD_REPO[(CI/CD Repository)]
        
        PROD_ORG -->|"sf hardis:org:retrieve:sources:retrofit"| RETROFIT_JOB
        RETROFIT_JOB -->|Creates PR| RETROFIT_PR
        RETROFIT_PR -->|Merges to| CICD_REPO
    end
    
    subgraph "MONITORING Process (Monitoring Repo)"
        PROD_ORG2[(Production Org)]
        BACKUP_JOB[Backup GitHub Action]
        MON_REPO[(Monitoring Repository)]
        
        PROD_ORG2 -->|"sf hardis:org:monitor:backup"| BACKUP_JOB
        BACKUP_JOB -->|Commits| MON_REPO
    end
    
    style CICD_REPO fill:#90EE90
    style MON_REPO fill:#87CEEB
```

| Feature | Retrofit (CI/CD) | Monitoring |
|---------|------------------|------------|
| **Source** | Production org directly | Production org directly |
| **Destination** | CI/CD repo (retrofitBranch) | Monitoring repo |
| **Scope** | Configurable metadata types | Full org metadata |
| **Action** | Creates PR for review | Direct commit |
| **Purpose** | Sync hotfixes back to dev | Audit trail & backup |
| **Schedule** | Configurable (often daily) | Nightly (or more frequent) |

### When to Use Monitoring Repository

| Use Case | Monitoring Repo Helps? |
|----------|----------------------|
| **"What changed in production yesterday?"** | ✅ Yes - git diff between commits |
| **"Who changed this Flow?"** | ✅ Yes - git blame + audit trail |
| **"Restore metadata from 2 weeks ago"** | ✅ Yes - git checkout previous commit |
| **"Sync hotfix to development branches"** | ❌ No - use Retrofit process |
| **"Deploy new feature to production"** | ❌ No - use CI/CD repository |
| **"Compare INT vs PROD metadata"** | ✅ Yes - compare branches |
...
```bash
# In VS Code with sfdx-hardis extension
# Or via command line:
  ...
  backup:
    ...
    steps:
      ...
      - name: Login to Salesforce
        run: sf hardis:auth:login
      - name: Backup metadata
        run: sf hardis:org:monitor:backup
      - name: Commit and push
        run: |
          git add --all
          git commit -m "Org state $(date -u +'%Y-%m-%d %H:%M') [skip ci]" || echo "No changes"
          git push
```

**Customizing backed-up metadata:**

```yaml
# .sfdx-hardis.yml in monitoring branch
monitoringDisable:
  - UNUSED_LICENSES  # Disable specific checks

# Or skip specific metadata types via env var:
# MONITORING_BACKUP_SKIP_METADATA_TYPES=Report,Dashboard
```

### Monitoring + CI/CD: Complete Architecture

```mermaid
flowchart TB
    subgraph "Development Flow (CI/CD Repo)"
        DEV[Developer] -->|Creates branch| FEATURE[feature/xyz]
        FEATURE -->|PR| INT[integration]
        INT -->|PR| UAT[uat]
        UAT -->|PR| PREPROD[preprod]
        PREPROD -->|PR| MAIN[main]
        MAIN -->|Deploy| PROD_ORG[(Production)]
    end
    
    subgraph "Retrofit Flow (CI/CD Repo)"
        PROD_ORG -->|Scheduled retrieve| RETROFIT[Retrofit Job]
        RETROFIT -->|PR to preprod| PREPROD
    end
    
    subgraph "Monitoring Flow (Monitoring Repo)"
        PROD_ORG -->|Nightly backup| MON[Monitoring Repo]
        MON -->|Notify| SLACK[Slack/Teams]
        MON -->|Visualize| GRAFANA[Grafana Dashboard]
    end
    
    style PROD_ORG fill:#FFD700
    style MON fill:#87CEEB
```

### Grafana Integration

sfdx-hardis monitoring can export metrics to Grafana for visualization:

```mermaid
flowchart LR
    subgraph "Monitoring Pipeline"
        BACKUP[Backup Job] --> METRICS[Collect Metrics]
        METRICS --> VECTOR[Vector.dev]
        VECTOR --> GRAFANA[Grafana]
    end
    
    subgraph "Dashboards"
        GRAFANA --> D1[Metadata Changes]
        GRAFANA --> D2[Org Limits]
        GRAFANA --> D3[Test Coverage]
        GRAFANA --> D4[API Version Usage]
    end
```

### Summary: Monitoring as a Safety Net

| Benefit | Description |
|---------|-------------|
| **Disaster Recovery** | Full metadata backup every night |
| **Audit Compliance** | Git history = complete change log |
| **Problem Detection** | Early warning for org issues |
| **Cross-Org Comparison** | Compare any two orgs easily |
| **Independent of CI/CD** | Works even without CI/CD adoption |

**Key Takeaway**: The monitoring repository provides a **safety net and audit trail** that complements (but does not replace) the CI/CD repository. It's particularly valuable for:
- Organizations not yet using CI/CD
- Detecting direct production changes (hotfixes)
